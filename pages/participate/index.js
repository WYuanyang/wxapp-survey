...
Page({
  ...
  querySurveyCount(){
    ...
    let participant = new AV.Query('participant');
    var survey = AV.Object.createWithoutData('survey', app.globalData.userId);
    let userInfo = app.globalData.userInfo;
    this.setData({userInfo});
    participant.equalTo('userId', survey);
    participant.count().then(function (surveyCount) {
  ...
  querySurveyList(){  
   ...
   // const cql = `select title,voteNums  from survey where owner = '${app.globalData.userId}' limit ${skip}  order by createdAt desc`; 
    const cql = `select include surveyId.owner,surveyId.title,surveyId.voteNums from participant where userId = pointer('survey','${app.globalData.userId}')  limit ${skip}  order by createdAt desc`;
    ...
    AV.Query.doCloudQuery(cql).then(function (data) {
        ...
        if(results.length > 0){
          ...
          let partIds = new Array();
          for (let i = 0; i < results.length;i++){
            surveyList.push(results[i].get('surveyId'));
            partIds[i] = results[i].get('objectId');
          }
          currPage.setData({ surveyList, partIds});
        }
  ...
  delPart(e){
    ...
    const cql = `delete from participant where objectId='${id}'`;
  ...
  getReachBottomData(){
    ...
    const cql = `select include surveyId.owner,surveyId.title,surveyId.voteNums from participant where userId = pointer('survey','${app.globalData.userId}') limit ${skip},${limit} order by createdAt desc`;
    AV.Query.doCloudQuery(cql).then(function (data) {
        // results 即为查询结果，它是一个 AV.Object 数组
        let results = data.results;
        if(results.length > 0){
         // let arr = currPage.setSurveyList(results);
          ...
          let arr1 = new Array();
          for (let i = 0; i < results.length; i++) {
            arr.push(results[i].get('surveyId'));
            arr1[i] = results[i].get('objectId');
          }
          let { surveyList, partIds} = currPage.data;
          surveyList = surveyList.concat(arr);
          partIds = partIds.concat(arr1);
          currPage.setData({ surveyList, partIds});
          wx.hideLoading();
        }
      }, function (error) {
